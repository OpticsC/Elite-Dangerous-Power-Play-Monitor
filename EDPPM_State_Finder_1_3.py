...
def fetch_edsm_generated_time(url):
    ...
    try:
        ...
        # Get text with line breaks so the "Url:" / "Generated:" blocks are readable
        txt = soup.get_text("\n", strip=True)

        # Prefer matching by full URL if possible, otherwise filename
        # We'll scan a small window after the match for "Generated:"
        patterns = [
            re.escape(url),
            re.escape(filename),
        ]
        ...
        for pat in patterns:
            ...
            if not m:
                continue
            window = txt[m.start(): m.start() + 600]  # plenty to include "Generated:"
            gm = re.search(r"Generated:\s*([A-Za-z]{3}\s+\d{1,2},\s+\d{4},\s+\d{1,2}:\d{2}:\d{2}\s+[AP]M)",
                           window)
            if gm:
        ...
        # If above didn't work, try a more structured scan:
        # Find the exact "Url: ..." line then look forward for "Generated:"
...
def system_has_ring_type_local(system, ring_type):
    ...
    for body in bodies:
        ...
        for ring in rings:
            rname = (ring.get("name") or "").lower()
            rtype = (ring.get("type") or "").lower()
            if ring_type == "icy":
                if "icy" in rtype or "icy" in rname:
                    return True
            elif ring_type == "metallic":
                if ("metallic" in rtype) or ("metal" in rtype) or ("metal" in rname):
                    return True
    return False
...
class EDPPMStateFinderApp:
    ...
    def _build_controls(self, parent):
        ...
        self.ring_filter = ttk.Combobox(parent, values=["None", "Icy Rings", "Metallic Rings"], state="readonly")
    ...
    def _update_local_data_status(self):
        # Show our stored "Generated" time if available; fall back to file mtime for display
    ...
    def load_or_download_dump(self, url, json_file):
        ...
        if not data:
            ...
            # => keep local, but inform user clearly
            if edsm_gen is None:
                self.set_status("Warning: Could not determine EDSM freshness (scrape/HEAD failed) — using local JSON")
                return data

            # If we have no local_gen stored, that means older versions / moved files.
            # In that case, FORCE one refresh if EDSM time exists (this prevents 'stuck old file' forever).
        ...
        # 4) Save EDSM timestamp to meta so we never rely on file mtime again
        if edsm_gen:
            save_local_generated(json_file, edsm_gen, source=src)
        else:
            # If EDSM time was missing but download succeeded, store "now" so we don't redownload every time
    ...
    def scan_loop(self):
        try:
            ...
            mode = self.mode_var.get()
            target_state = (self.state_combo.get() or "").strip().lower()
            selected_power = self.power_combo.get()

            ring_choice = self.ring_filter.get()
            ring_type = None
            if ring_choice == "Icy Rings":
                ring_type = "icy"
            elif ring_choice == "Metallic Rings":
                ring_type = "metallic"
